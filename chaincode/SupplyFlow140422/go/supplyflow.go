...
type BarleyLife struct {
    ObjectType string `json:"docType"` 
    BarleyID    string `json:"BarleyID"`
    Producer    string `json:"Producer"`
    SoilPH  string `json:"SoilPH"`
    GeoLocation string `json:"GeoLocation"`
}

type MaltLife struct {
    ObjectType string `json:"docType"` 
    MaltID  string `json:"MaltID"`
    QCPass  string `json:"QCPass"`
    BarleyOrderID  string `json:”BarleyOrderID”`
    BarleyOrder BarleyLife `json:"BarleyOrder"`
}

type BatchLife struct {
    ObjectType string `json:"docType"` 
    BatchID string `json:"BatchID"`
    QCPass  string `json:"QCPass"`
    MaltID  string `json:”MaltID”`
    MaltOrder MaltLife `json:"MaltOrder"`
}



type CaskLife struct {
    ObjectType string `json:"docType"` 
    CaskID string `json:”CaskID”`
    QCPass  string `json:"QCPass"`
    BatchID  string `json:”BatchID”`
...
func (s *SmartContract) InitBottling(ctx contractapi.TransactionContextInterface) error {
	...
	for i, s := range orderInput.CaskID{
		CaskAsBytes, err := ctx.GetStub().GetPrivateData("collectionWhiskeySupplyChain", ("CASK"+s))
		if err != nil {
...
/* func (s *SmartContract) BottleLife(ctx contractapi.TransactionContextInterface, BottleID string) (*BottleLife, error) {
	...
	for i, s := range bottle.CaskID{
		...
		casks []string := append(casks, cask)
	}
	//Get Batch Info 
...
/* func (s *SmartContract) BottleLife2(ctx contractapi.TransactionContextInterface, BottleID string) (*BottleLife, error) {

	lifecycle := new(BottleLife)
	...
	lifecycle.Casks = make([]CaskLife, 0)
	...
	for i := range lifecycle.Casks{
		BatchIDN := ("BATCH12")//+lifecycle.Casks[i].BatchID) 
		BatchJSON, err := ctx.GetStub().GetPrivateData("collectionWhiskeySupplyChain", BatchIDN)
		if err != nil {
			return nil, fmt.Errorf("failed to read from bottle %s", err.Error())
		}
		if BatchJSON == nil {
			return nil, fmt.Errorf("%s does not exist", BatchIDN, lifecycle.Casks[i], i)
		}
		batch := new(Distillation)
		_ = json.Unmarshal(BatchJSON, batch)

		lifecycle.Casks[i].Batch.BatchID=batch.BatchID
		//thisBatch.BatchID=batch.BatchID
		lifecycle.Casks[i].Batch.QCPass=batch.QCPass
		lifecycle.Casks[i].Batch.MaltID = batch.MaltOrderID
		///return nil, fmt.Errorf("Output:", batch, s)
	
	}

	//Get Malt Info 
	for i, s := range lifecycle.Casks{
		MaltIDN := ("MALT"+s.Batch.MaltID) 
		MaltJSON, err := ctx.GetStub().GetPrivateData("collectionWhiskeySupplyChain", MaltIDN)
		if err != nil {
			return nil, fmt.Errorf("failed to read from bottle %s", err.Error())
		}
		if MaltJSON == nil {
			return nil, fmt.Errorf("%s does not exist", MaltIDN, i, s.Batch.MaltID, s.Batch, s, lifecycle)
		}

		malt := new(MaltOrder)
		_ = json.Unmarshal(MaltJSON, malt)
		
		thisMalt := new(MaltLife)
		thisMalt.MaltID = malt.MaltOrderID
		thisMalt.QCPass = malt.QCPass
		thisMalt.BarleyOrderID = malt.BarleyOrderID

		s.Batch.MaltOrder = *thisMalt
	}


	//Get Barley Info 
	for i, s := range lifecycle.Casks{
		BarleyIDN := ("BARLEY"+s.Batch.MaltOrder.BarleyOrderID) 
		BarleyJSON, err := ctx.GetStub().GetPrivateData("collectionWhiskeySupplyChain", BarleyIDN)
		if err != nil {
			return nil, fmt.Errorf("failed to read from bottle %s", err.Error())
		}
		if BarleyJSON == nil {
			return nil, fmt.Errorf("%s does not exist", BarleyIDN, i)
		}

		barley := new(BarleyOrder)
		_ = json.Unmarshal(BarleyJSON, barley)

		thisBarley := new(BarleyLife)
		thisBarley.BarleyID = barley.BarleyOrderID
		thisBarley.Producer = barley.Producer
		thisBarley.SoilPH = barley.SoilPH
		thisBarley.GeoLocation = barley.GeoLocation

		s.Batch.MaltOrder.BarleyOrder = *thisBarley		
	}

	return lifecycle, nil
} */

func (s *SmartContract) BottleLife3(ctx contractapi.TransactionContextInterface, BottleID string) (*BottleLifeModel, error) {

...
func (s *SmartContract) SendToBottling(ctx contractapi.TransactionContextInterface, CaskID string) error {
	...
	if msp == "supplier-supply-com" {
		...
		if PrivOrderJSON == nil {
			return fmt.Errorf("%s does not exist", CaskID)
		}

		PrivOrderToUpdate := MaturationPrivate{}

		err = json.Unmarshal(PrivOrderJSON, &PrivOrderToUpdate) 
		if err != nil {
			return fmt.Errorf("failed to unmarshal JSON: %s", err.Error())
		}

		PrivOrderToUpdate.EndDate = time.Now().Format("2006-01-02")

		end, err := time.Parse("2006-01-02", PrivOrderToUpdate.EndDate)
		if err != nil {
			return err
		}
		start, err := time.Parse("2006-01-02", PrivOrderToUpdate.StartDate)
		if err != nil {
			return err
		}
		hours := end.Sub(start)
		//PrivOrderToUpdate.Age = fmt.Sprint("%.2f", (hours.Hours()/24/365))+" Years"
		PrivOrderToUpdate.Age = int((hours.Hours()/24/365))

		PrivorderJSONasBytes, err := json.Marshal(PrivOrderToUpdate)
		if err != nil {
			return fmt.Errorf(err.Error())
		}


		err = ctx.GetStub().PutPrivateData("collectionMaturationPrivate", OrderID, PrivorderJSONasBytes)
...
func (s *SmartContract) ReadCaskPrivate(ctx contractapi.TransactionContextInterface, CaskID string) (*MaturationPrivate, error) {
...
func (s *SmartContract) InitBottling(ctx contractapi.TransactionContextInterface) error {
	...
	for i, s := range orderInput.CaskID{
		...
		PrivOrder := MaturationPrivate{}
		PrivCaskAsBytes, err := ctx.GetStub().GetPrivateData("collectionMaturationPrivate", ("CASK"+s))
		err = json.Unmarshal(PrivCaskAsBytes, &PrivOrder) 
		if err != nil {
			return fmt.Errorf("failed to unmarshal JSON: %s", err.Error())
		}
		//Sets age to the youngest cask age in the bottle
		if Age == 0 {
			Age = PrivOrder.Age
		} else {

...
peer lifecycle chaincode queryinstalled -O json | jq -r '.installed_chaincodes | .[] | select(.package_id|startswith("supplyflow_4.8:"))' > ccstatus.json
...
#   --name supplyflow --version 4.8 --init-required --sequence $SEQUENCE --tls \

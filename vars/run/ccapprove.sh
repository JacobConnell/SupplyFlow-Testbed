...
peer lifecycle chaincode queryinstalled -O json | jq -r '.installed_chaincodes | .[] | select(.package_id|startswith("supplyflow_5.74:"))' > ccstatus.json
...
#   --name supplyflow --version 5.74 --init-required --sequence $SEQUENCE --tls \
